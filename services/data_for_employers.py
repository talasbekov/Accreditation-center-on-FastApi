...
statuses = ["в строю", "в отпуске", "на больничном", "в командировке", "прикомандирован", "откомандирован", "на дежурстве", "после дежурства", "на соревновании"]
...
def choose_status():
    if random.random() < 0.7:
        ...
        return random.choice(statuses[1:])  # Остальные 30% случайно среди других статусов
...
for department in departments:
    for _ in range(department["count_state"]):
        ...
        employee_data = create_employee(surname, firstname, patronymic, status, record_id)
        ...
        response = requests.post(api_url, headers=headers, json=employee_data, verify=False)

        if response.status_code == 201:  # Проверяем, успешно ли создан сотрудник
            print(f"Сотрудник {surname} {firstname} {patronymic} добавлен в {department['name']} со статусом {status}")
        else:
            print(f"Ошибка при добавлении сотрудника: {response.status_code}, {response.text}")
